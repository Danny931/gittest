...
import { LoadingOutlined, UnlockFilled, WarningFilled } from "@ant-design/icons";
...
export default function LockLiquidityModal({
  ...
  if (claimRewardStatus === UnlockRewardType.SWITCH_CHAIN) {
    content = (
      <div>
        ...
        <div className={classes.modaldes}>
          <div>Farming rewards can only be viewd and claimed on {getNetworkName()}.</div>
        ...
        <Button
          ...
          onClick={() => {
            if (!isMobile) {
              switchChain(Number(process.env.REACT_APP_CLAIM_ID), "");
            } else {
              closeModal();
            }
          }}
    ...
    titleText = " ";
  } else if (claimRewardStatus === UnlockRewardType.UNLUCKED_TOO_FREQUENTLY) {
    content = (
      <div>
        <div style={{ textAlign: "center" }}>
          <WarningFilled style={{ fontSize: 50, color: "#ffaa00", marginTop: 40 }} />
        </div>
        <div className={classes.modaldes}>Unlock too frequently. You may unlock again in X hours.</div>
        <Button
          type={buttonType}
          size="large"
          block
    ...
    titleText = " ";
  } else if (claimRewardStatus === UnlockRewardType.UNLUCKING) {
    // transferring
    content = (
      <>
        ...
        <div className={classes.detailItem}>
          <div className={classes.itemLeft}>
            <div>
              <img className={classes.itemContImg} src={getNetworkById(fromChainId).iconUrl} alt="" />
            </div>
            <div className={classes.itemText}>
              <div className={classes.itemTitle}>{getNetworkName()}</div>
            </div>
          </div>
          <div className={classes.itemRight}>
            ...
            {unlockableReward?.map(item => (
              <div className={classes.totalValueRN}>
                + {`${item?.amt?.toFixed(2)} ${getTokenSymbol(item?.token?.symbol, chainId)}`}
              </div>
            ))}
          </div>
        </div>
    ...
    titleText = " ";
  } else if (claimRewardStatus === UnlockRewardType.UNLUCK_SUCCESSED) {
    // transferring
    content = (
      <>
        <div className={classes.modalTop}>
          <div className={classes.modalTopIcon}>
            <UnlockFilled style={{ fontSize: 50, fontWeight: "bold" }} />
          </div>
        ...
        <div className={classes.modaldes}>
          ...
          {unlockableReward?.map((item, index) => {
            let claimableRewardsAmt = claimableReward ? claimableReward[index]?.amt || 0 : 0;
            return index === 0 ? (
              <span className={classes.greenText} style={{ fontSize: 14, fontWeight: 600 }}>
                {(item?.amt + claimableRewardsAmt).toFixed(2)}{" "}
                <span>{getTokenSymbol(item?.token?.symbol, chainId)}</span>
              </span>
            ) : (
              <span className={classes.greenText} style={{ fontSize: 14, fontWeight: 600 }}>
                {" "}
