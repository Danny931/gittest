...
import { TransferHistoryStatus, LPHistoryStatus, WithdrawDetail, TransferHistory } from "../../constants/type";
...
const TransferModal: FC<IProps> = ({ amount, receiveAmount, onCancel, onSuccess }) => {
  ...
  const {
    contracts: { bridge, originalTokenVault, peggedTokenBridge, fraxTokenBridge },
    transactor,
  } = useContractsContext();
  ...
  const tokenAddress = pegConfig?.getTokenBalanceAddress(selectedToken?.token?.address || "");
  ...
  const { isNativeETHToken } = useNativeETHToken(fromChain, selectedToken);
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      const transferTx = await executor().catch(e => {
  ...
  if (allowance && allowance.isZero() && !isNativeETHToken) {
    content = (
      <div className={classes.content}>
        ...
        />
        <div className={classes.modalToptext}>{getTokenSymbol(selectedToken?.token?.symbol, fromChain?.id)}</div>
        <div className={classes.modaldes} style={{ marginTop: 20 }}>
          Please approve {getTokenSymbol(selectedToken?.token?.symbol, fromChain?.id)} before you continue the
