...
import { Chain, Token, TokenInfo, GetTransferConfigsResponse } from "../../constants/type";
  ...
  return (
    <div className={styles.box}>
      <div className={styles.item}>
        ...
        <span className={styles.content}>
          {isMobile ? (
            ...
            <>
              1 {getTokenSymbol(selectedToken?.token?.symbol, fromChain?.id)} on{" "}
              <img className={styles.iconImg} src={fromChain?.icon} alt="" />{" "}
              {pegConfig.mode === PeggedChainMode.Off ? "≈" : "="} {bridgeRate}{" "}
              {getTokenDisplaySymbol(selectedToken?.token, toChain)} on{" "}
      ...
      <div className={styles.item}>
        ...
        <span className={styles.content}>
          {formatDecimalPart(totalFee, 8, "round", true)} {getTokenDisplaySymbol(selectedToken?.token, toChain)}
        </span>
      </div>
      {pegConfig.mode === PeggedChainMode.Off ? (
        <div className={styles.item}>
              ...
              title={
                <div className={styles.tooltipContent} style={{ textAlign: "center" }}>
                  {`You will receive at least ${
                    minimumReceived + getTokenDisplaySymbol(selectedToken?.token, toChain)
...
export const getTokenDisplaySymbol = (selectedToken: Token | undefined, toChain: Chain | undefined) => {
  const symbol = getTokenSymbol(selectedToken?.symbol ?? "", toChain?.id);
  return needToChangeTokenDisplaySymbol(selectedToken, toChain) ? "ETH" : symbol;
};
